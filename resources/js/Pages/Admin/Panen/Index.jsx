...
export default function Index({ dataPanens, profilLahans, flash }) {
    ...
    const handleSubmit = (e) => {
        ...
        post(route('admin.panen.store'), {
            onSuccess: () => {
                reset('berat_bersih_kg', 'harga_per_kg', 'profil_lahan_id');
                setModalTerbuka(false);
            }
        });
    };
        ...
        <>
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6">
                ...
                <div className="bg-white rounded-2xl border border-slate-200/80 shadow-sm overflow-hidden">
                    ...
                    {/* Header panel — sama persis dengan petani */}
                    <div className="p-6 border-b border-slate-200/80 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white">
                        ...
                        {/* ── Tombol + Catat Panen — sama style dengan tombol petani ── */}
                        <button
                            onClick={bukaModal}
                        ...
                        <table className="w-full text-left border-collapse">
                            <thead>
                                <tr className="bg-slate-50/75 text-slate-400 text-[11px] uppercase tracking-wider border-b border-slate-200/80">
                                    <th className="py-3 px-6 font-semibold">Tanggal / Petani</th>
                                    <th className="py-3 px-6 font-semibold">Timbangan</th>
                                    <th className="py-3 px-6 font-semibold">Bruto</th>
                                    <th className="py-3 px-6 font-semibold">Alokasi Simpanan</th>
                                    <th className="py-3 px-6 font-semibold text-amber-500">Jatah Bersih</th>
                                    <th className="py-3 px-6 font-semibold">Aksi</th>
                                </tr>
                            </thead>
                                        ...
                                        <tr key={panen.id} className="hover:bg-slate-50/50 transition-colors">
                                            <td className="py-4 px-6 whitespace-nowrap">
                                                <p className="font-semibold text-slate-900">
                                                    {new Date(panen.tanggal_panen).toLocaleDateString('id-ID', {
                                            ...
                                            <td className="py-4 px-6 whitespace-nowrap">
                                                ...
                                                <p className="text-xs text-slate-400 mt-0.5">
                                                    @{formatRupiah(panen.harga_per_kg)}/Kg
                                                </p>
                                            </td>
                                            ...
                                            <td className="py-4 px-6 whitespace-nowrap">
                                                <div className="inline-flex flex-col gap-1.5 text-left">
                                                    ...
                                                    <div className="flex items-center justify-between bg-blue-50/40 px-2.5 py-1 rounded-lg border border-blue-100/60">
                                                        <span className="text-[11px] font-bold text-blue-800 uppercase tracking-wide">🧪 Racun</span>
            ...
            {/* ── MODAL INPUT PANEN — sama style dengan modal petani ── */}
            {modalTerbuka && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-0">
                    {/* Overlay — sama dengan petani */}
                    <div
                    ...
                    <div className="relative bg-white rounded-2xl shadow-xl w-full max-w-lg overflow-hidden transform transition-all flex flex-col max-h-[90vh]">
                        ...
                        {/* Modal Header — sama persis dengan petani */}
                        <div className="flex justify-between items-center p-5 border-b border-slate-200/80 bg-slate-50 shrink-0">
                            <div>
                                <h3 className="text-base font-bold text-slate-900 tracking-tight">
                                    ⚖️ Catat Hasil Panen Baru
                                </h3>
                                <p className="text-xs text-slate-400 mt-0.5">
                                    Isi data timbangan untuk memproses pemotongan sinking fund otomatis.
                                </p>
                            </div>
                            <button
                                onClick={tutupModal}
                            ...
                            <form id="panenForm" onSubmit={handleSubmit} className="space-y-6">
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div>
                                        ...
                                        {errors.profil_lahan_id && (
                                            <p className="text-red-500 text-xs mt-1">{errors.profil_lahan_id}</p>
                                        )}
                                    </div>
                                </div>

                                {/* SECTION 2 — Tanggal & Timbangan */}
                                <div className="space-y-4">
                                    <h4 className="text-xs font-bold text-slate-400 uppercase tracking-wider flex items-center gap-2 border-b border-slate-100 pb-1.5">
                                    ...
                                    <div>
                                        ...
                                        {errors.tanggal_panen && (
                                            <p className="text-red-500 text-xs mt-1">{errors.tanggal_panen}</p>
                                        )}
                                    </div>
                                    <div className="grid grid-cols-2 gap-4">
                                        <div>
                                            ...
                                            <input
                                                ...
                                                required
                                            />
                                            {errors.berat_bersih_kg && (
                                                <p className="text-red-500 text-xs mt-1">{errors.berat_bersih_kg}</p>
                                            )}
                                        </div>
                                        <div>
                                            <label className="block text-xs font-semibold text-slate-500 uppercase tracking-wide mb-1">
                                            ...
                                            />
                                            {errors.harga_per_kg && (
                                                <p className="text-red-500 text-xs mt-1">{errors.harga_per_kg}</p>
                                            )}
                                        </div>
                                    </div>
                                </div>

                                {/* SECTION 3 — Simulasi live (muncul kalau total > 0) */}
                                {total > 0 && (
                                    <div className="space-y-4">
                                        <h4 className="text-xs font-bold text-slate-400 uppercase tracking-wider flex items-center gap-2 border-b border-slate-100 pb-1.5">
                        ...
                        {/* Modal Footer — sama persis dengan petani */}
