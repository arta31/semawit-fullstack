        ...
        <>
                                ...
                                {logs.data.length > 0 ? (
                                    logs.data.map((log) => (
                                        <tr key={log.id} className="hover:bg-slate-50/50 transition-colors">
                                            ...
                                            <td className="py-4 px-6 whitespace-nowrap">
                                                <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 text-[11px] font-bold uppercase rounded-full tracking-wide ${
            ...
            {isModalOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-0">
                    <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm transition-opacity" onClick={handleCloseModal} />
                            ...
                            <form id="perawatanForm" onSubmit={handleSubmit} className="space-y-6">
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div>
                                        ...
                                        <select value={data.profil_lahan_id} onChange={e => setData('profil_lahan_id', e.target.value)} className="block w-full rounded-xl border-slate-200 focus:border-emerald-500 focus:ring-emerald-500 text-sm shadow-sm bg-white" required>
                                            ...
                                            {lahans.map((lahan) => (
                                                <option key={lahan.id} value={lahan.id}>{lahan.user.name} — {lahan.nama_lahan}</option>
                                            ))}
                                        </select>
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div className="flex gap-3">
                                        <label className={`flex-1 flex items-center justify-center gap-2 p-3 rounded-xl border-2 cursor-pointer transition-all duration-150 ${data.jenis_perawatan === 'pupuk' ? 'border-emerald-500 bg-emerald-50/50 shadow-sm' : 'border-slate-100 hover:border-slate-200 bg-white'}`}>
                                            <input type="radio" name="jenis_perawatan" value="pupuk" checked={data.jenis_perawatan === 'pupuk'} onChange={e => setData('jenis_perawatan', e.target.value)} className="hidden" />
                                            <Package size={18} className="text-emerald-600" />
                                            <span className="font-bold text-sm text-slate-800">Tabungan Pupuk</span>
                                        </label>
                                        <label className={`flex-1 flex items-center justify-center gap-2 p-3 rounded-xl border-2 cursor-pointer transition-all duration-150 ${data.jenis_perawatan === 'racun' ? 'border-amber-500 bg-amber-50/50 shadow-sm' : 'border-slate-100 hover:border-slate-200 bg-white'}`}>
                                            <input type="radio" name="jenis_perawatan" value="racun" checked={data.jenis_perawatan === 'racun'} onChange={e => setData('jenis_perawatan', e.target.value)} className="hidden" />
                                ...
                                {data.profil_lahan_id && (
                                    <div className="space-y-4">
                                        ...
                                        <div className={`rounded-xl p-4 border-2 transition-all duration-300 ${isSaldoKurang ? 'bg-red-50 border-red-200' : 'bg-emerald-50/50 border-emerald-100'}`}>
                                            ...
                                            {isSaldoKurang && (
                                                <div className="mt-3 bg-red-100 border border-red-200 rounded-lg p-3 text-center">
                                                    <p className="text-xs font-bold text-red-700 flex items-center justify-center gap-1.5">
                                                        <XCircle size={14} /> SALDO TIDAK CUKUP — Kurang {formatRupiah(totalEstimasi - saldoAktif)}
                                                    </p>
